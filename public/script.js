...
// Helper to apply contact configuration to DOM elements
...
// Fetch configuration from /api/config with graceful local fallback
fetch('/api/config')
    .then(response => {
        if (!response.ok) throw new Error("API not available");
        return response.json();
    })
    .then(config => applyContactConfig(config))
    .catch(err => {
        console.warn('API config unavailable, applying standard contact fallback:', err.message);
        applyContactConfig({
            CONTACT_NUMBER: "+919884087878",
            DISPLAY_CONTACT_NUMBER: "+91 9884087878",
            EMAIL_ADDRESS: "trustcareelectronics@gmail.com",
            MAP_LOCATION_URL: "https://maps.app.goo.gl/U5BKf7q1mpoMP46T9"
        });
    });
